...
### Reference-gap reviews and generic pteridine searches

`terpedia-489015.terpedia_core.cannabis_phase1_reference_gap_bundle_20260905_v1`
stores nine complete source documents and one metadata record. The four string
columns are `record_kind`, `record_id`, `record_json`, and `report_sha256`.
Source-document IDs are repository paths; their JSON includes the complete
document and its source-file SHA-256. This preserves nested search rows,
sequences, evidence pointers, uncertainty and proposed tests without flattening
away their context. The lossless local export is
`data/derived/phase1-reference-gap-bundle.ndjson`; its report is
`data/reports/phase1-reference-gap-bundle.json`.

The bundle includes exact-family unreviewed plant/nonplant reference lookups,
glucuronolactone abstract and full-text reviews, and the biopterin reference
review plus two complete proteome searches and their reference-discovery reports.
Neither exact-family follow-up returned an annotated reference. The human QDPR
search and the broader seven-reviewed-reference pteridine search each screened
all 30,304 pinned Cannabis proteins and returned no alignments at E-value 1e-5.
These negative results do not establish absence of the activity. The pteridine
references have generic substrates and remain ineligible for exact biopterin
reaction assignment without further evidence.

The glucuronolactone full-text review retains the distinction between the paper's
cyclic hemiacetal and the catalog's open-aldehyde lactone identity. Its aqueous
interconversion observations do not establish the exact catalog reaction in
Cannabis tissue. No candidate model, target identity, route count or atom-tracing
status changed in this publication batch.
