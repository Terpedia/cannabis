...
- Structurally inferred reaction-product carbon mappings: **3,549** (**15.9951%**).
- Additional reaction-product carbons are explicitly classified as **16,970
  ambiguous** and **1,669 unresolved**; every product carbon has a mapping row.
...
Terpedia snapshot, 7 carbon nodes are reachable from the CO₂ seed through
2,594 inferred structural edges plus 1 candidate CO₂-transfer edge. This uses
22 explicit Terpedia physiological right-to-left direction assertions plus the
source-backed Rubisco directional record. There are
643 carbon-containing reactant entities outside that lineage and therefore
reported as external-carbon-source blockers. The current target summary is 0 supported, 0 candidate, and 6,220
unresolved CannabisDB compounds. This indicates that the imported reaction
network is missing the CO₂-assimilation and/or structure-resolved links needed
to establish the plant-wide carbon path; it is not evidence that Cannabis uses
