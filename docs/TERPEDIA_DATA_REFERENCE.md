...
- Terpedia has **1,099** working reactions, including **6** explicitly
  non-enzymatic decarboxylations; **7** of **1,093** enzyme-requiring
  reactions lack a direct enzyme association.
- All **7** have attached candidate proteins; the remaining gap is candidate
  enzyme validation rather than candidate discovery.
- CannabisDB compounds: **6,220**.
- CannabisDB carbon atoms: **285,623**.
- Terpedia reaction-product carbon atoms: **22,522**.
- Structurally inferred reaction-product carbon mappings: **4,098** (**18.20%**).
- Additional reaction-product carbons are explicitly classified as **186
  candidate**, **16,813 ambiguous**, and **1,425 unresolved**; every product
...
- CannabisDB-to-Terpedia identity crosswalk: **148 exact**, **1 ambiguous**,
  **6,070 CannabisDB compounds without an exact link**; 1,408 CannabisDB
  carbon atoms are identity-linked. The reverse-direction Terpedia inventory
  has 1,108 metabolites without a CannabisDB match. A separate connectivity
  candidate layer contains 250 one-to-one candidates and 40 ambiguous groups;
  these may differ in stereochemistry, protonation, or tautomer state and are
  never treated as exact identity.
...
working network, 2,896 carbon atom nodes across the reachable metabolite
entities are reachable from the CO₂ seed through 3,104 inferred structural
edges and 98,118 explicitly retained candidate alternative edges. Candidate alternatives
...
current target summary is 1 supported, 220 candidate, and 5,999 unresolved
CannabisDB compounds. The one supported target is the CannabisDB CO₂ record
itself; 211 additional targets have exact or candidate identities with partial
...
CannabisDB carbon atoms into **1 supported**, **1,162 candidate**, and
**284,460 unresolved** atoms. Each group retains CannabisDB atom indices,
...
1,261 working Terpedia metabolite records, 1,099 working biochemical reaction records,
all reaction participants and stoichiometric coefficients, enzyme associations,
source URLs, and the 148 exact CannabisDB–Terpedia identity links. The two
...
NetworkDB also carries 250 connectivity-level candidate identity links and 40
...
Every one of the 1,099 working reaction records also carries a carbon-mapping
summary from the RDKit report: 48 reactions are fully inferred, 24 retain
candidate mappings, 496 retain ambiguous carbon mappings, 460 retain
...
sequence searches against reviewed reference proteins. Twenty-one candidate
...
reaction set contains 2,249 records: 1,719 candidate records and 530 blocked
records; 1,332 are attached to a working reaction, including six explicit
...
5,999 unresolved CO₂-lineage targets. TKS and OAC have dedicated coupled or
...
priority, 148 exact-identity records are medium priority, and the remainder
...
- 719 of 1,099 working reactions are fully element- and charge-balanced after
...
6,220 CannabisDB compounds (285,623 carbon atoms), all 1,099 reaction mapping
rows, and the no-imbalanced-reactions check. This is an artifact-completeness
gate, not evidence that every reaction is biologically active.
