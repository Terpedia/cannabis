...
- `data/reports/pubchem-resolution.json` is an exact-InChIKey PubChem
  cross-reference for every CannabisDB record. It retains resolved CIDs and
  explicit no-match records; PubChem identity is evidence for structure
  reconciliation only, not evidence of Cannabis biosynthesis.
