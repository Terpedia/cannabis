...
def run():
    paths = [Path('data/reports', n + '.json') for n in ('phase1-expanded-candidate-net',
        ...
        'phase1-candidate-direction-review')]
    ...
    for path, sha in review['source_sha256'].items():
        if hashlib.sha256(Path(path).read_bytes()).hexdigest() != sha:
            raise ValueError('Direction review source mismatch')
    bundle = build(reports[0], reports[1:4] + [reports[0]])
    bundle['view_scenario'] = 'expanded-candidates'
    bundle['view_boundary'] = 'Re-solved candidate-only network: 108 target certificates, seven more than baseline. All seven added routes use newly linked reactions opposite to their source-written equations; reverse activity remains unestablished. Source ordering alone is not a physiological constraint. This is not the frozen full-catalog annotation count.'
    reviews = {r['reaction_id']: r for r in review['reviews']}
    bundle['reactions'] = [{**r, 'is_new_catalog_candidate': r['new_catalog_candidate'],
