...
def build_map_snapshot(networkdb_path: Path, output: Path, lineage_path: Path | None = None, focus_output: Path | None = None) -> dict:
    ...
    identity_candidate_upstream_connections = networkdb.get("identity_candidate_upstream_connections", [])
    hypothesis_connected_ids = {h[side] for h in hypothetical_connections for side in ("substrate_compound_id", "product_compound_id")}
    identity_candidate_connected_ids = {h[side] for h in identity_candidate_upstream_connections for side in ("source", "target")}
    ...
    compounds = [{**c, "reaction_connected": c.get("id") in reaction_connected_ids, "hypothesis_connected": c.get("id") in hypothesis_connected_ids or c.get("namespace") == "terpedia_hypothesis_product" or c.get("id") in identity_candidate_connected_ids, "co2_reachable": c.get("id") in reachable_entities or bool(c.get("co2_reachable_carbon_atoms"))} for c in networkdb.get("compounds", [])]
    reactions = []
    ...
    report = {"schema": "cannabis-carbon.network-map.v1", "source_networkdb": str(networkdb_path), "source_lineage": str(lineage_path) if lineage_path else None, "claim_boundary": "This is a compact visualization projection containing the complete compound inventory, balanced reaction records, and separate hypothesis-edge layers. It is not proof of in-vivo flux.", "compounds": compounds, "reactions": reactions, "hypothetical_connections": hypothetical_connections, "identity_candidate_upstream_connections": identity_candidate_upstream_connections, "coverage": networkdb.get("coverage", {}), "focus": {"co2_reachable_compounds": sum(c["co2_reachable"] for c in compounds), "reaction_connected_compounds": sum(c["reaction_connected"] for c in compounds), "hypothesis_connected_compounds": sum(c["hypothesis_connected"] for c in compounds), "all_inventory_compounds": len(compounds)}}
    ...
    if focus_output:
        ...
        focused_identity_candidates = [
            h for h in identity_candidate_upstream_connections
            if h["source"] in focused_ids or h["target"] in focused_ids
        ]
        focused_ids.update(
        ...
        focused_ids.update(h["source"] for h in focused_identity_candidates)
        focused_ids.update(h["target"] for h in focused_identity_candidates)
        focused_reactions = [
        ...
        focused = {**report, "schema": "cannabis-carbon.network-map-focus.v1", "compounds": [c for c in compounds if c["id"] in focused_ids], "reactions": focused_reactions, "hypothetical_connections": [h for h in focused_hypotheses if h["substrate_compound_id"] in focused_ids and h["product_compound_id"] in focused_ids], "identity_candidate_upstream_connections": [h for h in focused_identity_candidates if h["source"] in focused_ids and h["target"] in focused_ids], "focus": {**report["focus"], "focused_compounds": len(focused_ids), "focused_hypothesis_edges": len(focused_hypotheses), "focused_identity_candidate_upstream_edges": len(focused_identity_candidates)}}
...
def build_networkdb(network_path: Path, compounds_path: Path, crosswalk_path: Path, output: Path, hypotheses_path: Path | None = None, genome_search_path: Path | None = None, genome_fasta_path: Path | None = None, mapping_path: Path | None = None, lineage_path: Path | None = None, atom_audit_path: Path | None = None, pubchem_path: Path | None = None, identity_set_path: Path | None = None, hypothetical_connections_path: Path | None = None, hypothetical_reactions_path: Path | None = None, hypothesis_enzyme_evidence_path: Path | None = None, hypothesis_enzyme_catalog_path: Path | None = None, hypothesis_balance_path: Path | None = None, identity_set_bridges_path: Path | None = None, identity_set_connectivity_upstream_path: Path | None = None) -> dict:
    ...
    identity_set_candidates_by_id = {}
    for record in (identity_set or {}).get("candidate_records", []):
        identity_set_candidates_by_id.setdefault(record["cannabisdb_id"], []).append(record)
    ...
    identity_set_bridges = json.loads(identity_set_bridges_path.read_text()) if identity_set_bridges_path and identity_set_bridges_path.exists() else None
    identity_set_connectivity_upstream = json.loads(identity_set_connectivity_upstream_path.read_text()) if identity_set_connectivity_upstream_path and identity_set_connectivity_upstream_path.exists() else None
    ...
    for compound in catalog:
        ...
        identity_set_record = identity_set_by_id.get(compound["id"])
        identity_set_candidates = identity_set_candidates_by_id.get(compound["id"], [])
        compounds.append({**compound, "namespace": "cannabisdb", "identity_link": identity_by_cdb.get(compound["id"]), "identity_link_candidates": candidates, "identity_status": "exact" if compound["id"] in identity_by_cdb else "candidate" if candidates else "unresolved", "terpedia_identity_set": identity_set_record, "terpedia_identity_set_candidates": identity_set_candidates, "terpedia_identity_set_status": "exact-inchikey" if identity_set_record else "candidate-connectivity-inchikey" if identity_set_candidates else "unmatched", "terpedia_identity_set_terpene_id": (identity_set_record or {}).get("terpedia_identity", {}).get("terpene_id"), "terpedia_identity_set_candidate_terpene_ids": sorted({r.get("terpedia_identity", {}).get("terpene_id") for r in identity_set_candidates if r.get("terpedia_identity", {}).get("terpene_id")}), "terpedia_identity_set_source_count": len((identity_set_record or {}).get("terpedia_identity", {}).get("source_crossrefs", [])), "terpedia_identity_set_candidate_count": len(identity_set_candidates), "pubchem": pubchem_record.get("pubchem"), "pubchem_status": pubchem_record.get("status", "not-queried"), "pubchem_reason": pubchem_record.get("reason"), "carbon_lineage_status": target.get("status", "unresolved"), "co2_reachable_carbon_atoms": target.get("reachable_carbon_atoms", 0), "carbon_lineage_reason": target.get("reason", "no-lineage-record")})
    ...
    if identity_set_connectivity_upstream:
        existing_compound_ids = {compound["id"] for compound in compounds}
        for row in identity_set_connectivity_upstream.get("rows", []):
            precursor_id = row.get("precursor_terpene_id")
            precursor_node_id = f"terpedia:identity-set-candidate:{precursor_id}"
            if precursor_id and precursor_node_id not in existing_compound_ids:
                compounds.append({"id": precursor_node_id, "namespace": "terpedia_identity_set_candidate", "label": precursor_id, "smiles": row.get("precursor_smiles"), "inchikey": row.get("precursor_inchikey"), "formula": row.get("precursor_molecular_formula"), "carbon_atom_count": row.get("precursor_carbon_count"), "identity_status": "identity-set-candidate-precursor", "source_url": "https://console.cloud.google.com/bigquery?project=terpedia-489015", "claim_boundary": "Candidate precursor structure from Terpedia identity set; not confirmed Cannabis metabolism."})
                existing_compound_ids.add(precursor_node_id)
            for target_id in row.get("candidate_cannabisdb_ids", []):
                identity_candidate_upstream_connections.append({"id": f"identity-candidate:{row.get('reaction_id')}:{precursor_node_id}>{target_id}", "source": precursor_node_id, "target": target_id, "label": f"connectivity candidate · {row.get('reaction_id')}", "status": "candidate", "layer": "Terpedia MARTS connectivity identity hypothesis", "reaction_id": row.get("reaction_id"), "source_url": row.get("source_url"), "source_uniprot_id": row.get("source_uniprot_id"), "source_ec_number": row.get("source_ec_number"), "reaction_smarts": row.get("reaction_smarts"), "product_terpene_id": row.get("product_terpene_id"), "claim_boundary": row.get("claim_boundary")})
        identity_candidate_upstream_connections.sort(key=lambda edge: edge["id"])
    ...
    if identity_set_bridges:
        for bridge in identity_set_bridges.get("bridges", []):
            hypothetical_connections.append({
                "reaction_id": bridge.get("reaction_id"),
                "substrate_compound_id": bridge["core_entity_id"],
                "product_compound_id": bridge["cannabisdb_id"],
                "source_type": bridge.get("source_type"),
                "evidence_type": bridge.get("evidence_type"),
                "source_url": bridge.get("source_url"),
                "reaction_smarts": bridge.get("reaction_smarts"),
                "status": "candidate",
                "layer": "Terpedia identity-set core bridge",
                "identity_set_core_bridge": True,
                "precursor_identity_id": bridge.get("precursor_identity_id"),
                "product_identity_id": bridge.get("product_identity_id"),
                "target_pair_carbon_mapping": bridge.get("target_pair_carbon_mapping"),
                "claim_boundary": bridge.get("claim_boundary"),
            })
    ...
    report = {"schema": "cannabis-carbon.networkdb.v1", "sources": {"cannabisdb_compounds": str(compounds_path), "terpedia_network": str(network_path), "identity_crosswalk": str(crosswalk_path), "terpedia_identity_set": str(identity_set_path) if identity_set_path else None, "terpedia_identity_set_connectivity_upstream": str(identity_set_connectivity_upstream_path) if identity_set_connectivity_upstream_path else None, "pubchem_resolution": str(pubchem_path) if pubchem_path else None, "candidate_hypotheses": str(hypotheses_path) if hypotheses_path else None, "genome_search": str(genome_search_path) if genome_search_path else None, "carbon_mapping": str(mapping_path) if mapping_path else None, "carbon_lineage": str(lineage_path) if lineage_path else None, "carbon_atom_audit": str(atom_audit_path) if atom_audit else None, "identity_set_core_bridges": str(identity_set_bridges_path) if identity_set_bridges_path else None}, "compounds": compounds, "reactions": reactions, "identity_links": crosswalk["matches"], "identity_link_candidates": crosswalk.get("candidate_matches", []), "candidate_proteins": list(candidate_protein_records.values()), "candidate_hypotheses": hypothesis_items, "genome_search": genome_search, "carbon_atom_audit": {"source": str(atom_audit_path), "carbon_atoms_total": atom_audit.get("carbon_atoms_total"), "status_counts": atom_audit.get("status_counts")} if atom_audit else None, "identity_candidate_upstream_connections": identity_candidate_upstream_connections, "coverage": {"cannabisdb_compounds": len(catalog), "terpedia_metabolites": sum(e.get("type") == "metabolite" for e in network["entities"]), "compound_records": len(compounds), "terpedia_reactions": len(reactions), "reaction_records": len(reactions), "exact_identity_links": len(crosswalk["matches"]), "ambiguous_identity_links": crosswalk["ambiguous"], "terpedia_unmatched_metabolites": crosswalk.get("terpedia_unmatched", crosswalk["unmatched"]), "unmatched_cannabisdb_compounds": crosswalk.get("cannabisdb_unmatched", crosswalk["unmatched"]), "connectivity_candidate_identity_links": len(crosswalk.get("candidate_matches", [])), "connectivity_candidate_ambiguous": len(crosswalk.get("candidate_ambiguous_records", [])), "terpedia_identity_set_matches": len(identity_set_by_id), "terpedia_identity_set_connectivity_candidates": len(identity_set_candidates_by_id), "terpedia_identity_set_connectivity_candidate_records": sum(len(v) for v in identity_set_candidates_by_id.values()), "terpedia_identity_set_connectivity_upstream_connections": len(identity_candidate_upstream_connections), "external_id_coverage": {"records_with_any_external_id": sum(bool(c.get("external_ids")) for c in catalog), "records_without_external_ids": sum(not c.get("external_ids") for c in catalog), "counts_by_database": dict(sorted(external_id_counts.items()))}, "carbon_mapping_reactions": len(mapping_by_reaction), "carbon_mapping_status_counts": {status: sum(r["carbon_mapping"]["status"] == status for r in reactions) for status in ("inferred", "candidate", "ambiguous", "unresolved", "unavailable")}, "carbon_lineage_edge_totals": lineage_edge_totals, "carbon_lineage_target_status_counts": lineage_status_counts, "candidate_hypotheses": len(hypothesis_items), "candidate_protein_records": len(candidate_protein_records), "candidate_proteins_with_sequence": sum(bool(p.get("sequence_search", {}).get("sequence_present")) for p in candidate_protein_records.values()), "candidate_proteins_with_diamond_hits": sum(bool((p.get("diamond_search") or {}).get("hits")) for p in candidate_protein_records.values()), "candidate_proteins_meeting_strong_hit_threshold": sum(bool((p.get("diamond_search") or {}).get("strong_candidate_hit")) for p in candidate_protein_records.values()), "reactions_with_candidate_proteins": sum(bool(r["candidate_proteins"]) for r in reactions), "reactions_with_exact_ec_annotation_candidates": sum(any(p.get("candidateOrigin") == "Terpedia reaction EC to protein exact-EC join" for p in r["candidate_proteins"]) for r in reactions)}, "claim_boundary": "This is a unified inventory and reaction database. Candidate identity-set upstream connections are source-linked hypotheses only; presence of a compound, reaction, enzyme association, or identity link does not establish in-vivo Cannabis biosynthesis."}
    output.parent.mkdir(parents=True, exist_ok=True)
    ...
    report["coverage"]["identity_set_core_bridge_edges"] = sum(bool(connection.get("identity_set_core_bridge")) for connection in hypothetical_connections)
    report["coverage"]["identity_set_core_bridge_targets"] = len({connection.get("product_compound_id") for connection in hypothetical_connections if connection.get("identity_set_core_bridge")})
    report["coverage"]["identity_set_core_bridge_core_entities"] = len({connection.get("substrate_compound_id") for connection in hypothetical_connections if connection.get("identity_set_core_bridge")})
