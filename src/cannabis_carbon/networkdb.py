...
def build_networkdb(network_path: Path, compounds_path: Path, crosswalk_path: Path, output: Path, hypotheses_path: Path | None = None, genome_search_path: Path | None = None) -> dict:
    ...
    for item in hypotheses.get("items", []):
        if item.get("reaction_id"):
            candidate_by_reaction.setdefault(item["reaction_id"], []).extend({**p, "sequence_search": genome_by_protein.get(p.get("proteinId"), {}).get("sequence_search"), "diamond_search": genome_by_protein.get(p.get("proteinId"), {}).get("diamond_search")} for p in item.get("candidate_proteins", []))
    reactions = []
    for entity in network["entities"]:
    ...
    for protein_id, record in list(candidate_protein_records.items()):
        if protein_id in genome_by_protein:
            candidate_protein_records[protein_id] = {**record, "sequence_search": genome_by_protein[protein_id].get("sequence_search"), "diamond_search": genome_by_protein[protein_id].get("diamond_search")}
