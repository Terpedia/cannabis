...
def build_reaction_report(source: Path, output: Path) -> dict:
    ...
    report = {"schema": "cannabis-carbon.reaction-carbon-mapping.v1", "source": str(source), "reaction_count": len(rows), "status_counts": {status: sum(row["status"] == status for row in rows) for status in ("inferred", "unresolved")}, "carbon_counts": {"product_carbon_atoms": product_carbons, "mapped_carbon_atoms": mapped, "ambiguous_carbon_atoms": ambiguous, "unresolved_carbon_atoms": unresolved, "unresolved_or_ambiguous_carbon_atoms": ambiguous + unresolved, "mapping_coverage_percent": round(100 * mapped / product_carbons, 4) if product_carbons else 0.0}, "reactions": rows, "claim_boundary": "Structural atom mapping is not isotope tracing and does not establish in-vivo flux."}
