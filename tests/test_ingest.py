...
def test_ingest_preserves_source_compound_names(tmp_path):
    source = tmp_path / "one.sdf"
    writer = Chem.SDWriter(str(source))
    mol = Chem.MolFromSmiles("CCO")
    mol.SetProp("DATABASE_ID", "CDBTEST")
    mol.SetProp("GENERIC_NAME", "ethanol")
    mol.SetProp("JCHEM_IUPAC", "ethanol")
    writer.write(mol)
    writer.close()
    graph = tmp_path / "graph.json"
    ingest_sdf(source, graph, tmp_path / "report.json")
    compound = __import__("json").loads(graph.read_text())["compounds"][0]
    assert compound["label"] == "ethanol"
    assert compound["aliases"] == ["ethanol"]
