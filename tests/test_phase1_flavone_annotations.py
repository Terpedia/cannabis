...
def test_flavone_annotations_cover_every_screened_lead_and_reference(monkeypatch):
    monkeypatch.chdir(Path(__file__).resolve().parents[1])
    report = json.loads(Path('data/reports/phase1-flavone-annotations.json').read_text())
    ...
    assert len(report['rows']) == len(sequences) == 33
