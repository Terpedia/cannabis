...
test('every net certificate projects all participants, coefficients, extents and candidate evidence', () => {
  let count = 0;
...
test('controls retain full balances, clear gaps, highlight without hiding, and recover from load errors', async () => {
  class Field {
  ...
  const ids=['netCy','netFit','poolHighlight','netSearch','netScope','netTarget','netReaction','netRetry','netCounts','netBalance','netEquation','netSources','netEvidence','netDetails','netTitle','netStatus','netMessage','netMetrics','netMatches'];
  ...
  let fail=true;
  const ui={URLSearchParams,location:{search:''},document:{getElementById:id=>fields[id],createElement:()=>new Field()},cytoscape:()=>cy,
