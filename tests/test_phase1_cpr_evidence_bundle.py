...
from cannabis_carbon.phase1_cpr_evidence_bundle import SOURCES
from cannabis_carbon.phase1_reference_gap_bundle import build


def test_cpr_bundle_is_lossless_without_promotion(monkeypatch):
    monkeypatch.chdir(Path(__file__).resolve().parents[1])
    path = Path('data/reports/phase1-cpr-evidence-bundle.json')
    ...
    assert Path('docs/data/cpr-evidence-bundle.json').read_bytes() == path.read_bytes()
    rows = [json.loads(line) for line in Path('data/derived/phase1-cpr-evidence-bundle.ndjson').read_text().splitlines()]
    assert len(rows) == len({(r['record_kind'], r['record_id']) for r in rows}) == 6
