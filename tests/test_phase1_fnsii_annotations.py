...
def test_fnsii_annotations_preserve_all_sequences_and_original_evidence(monkeypatch):
    monkeypatch.chdir(Path(__file__).resolve().parents[1])
    report = json.loads(Path('data/reports/phase1-fnsii-annotations.json').read_text())
    search = json.loads(Path('data/reports/phase1-fnsii-search.json').read_text())
    sequences = {p['accession']: p['sequence'] for p in search['reference_sequences'] + search['cannabis_candidates']}
    assert len(report['rows']) == len(sequences) == 98
