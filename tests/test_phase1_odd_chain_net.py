...
def test_full_inventory_exact_replay_and_changed_obstructions():
    root = Path('data/reports')
    read = lambda n: json.loads((root / ('phase1-' + n + '.json')).read_bytes())
    report = read('odd-chain-net'); parent = read('alkane-net')
    ...
    assert added == {r['id']: r for r in read('odd-chain-elongation')['reactions']}
    ...
    assert report['summary']['balanced_equations'] == 18211
    ...
    for obstruction in read('alkane-obstructions')['targets']:
