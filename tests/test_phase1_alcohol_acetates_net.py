...
def test_full_inventory_acetylation_certificates_and_direction_bounds():
    root = Path('data/reports')
    read = lambda n: json.loads((root / ('phase1-' + n + '.json')).read_bytes())
    report = read('alcohol-acetates-net'); parent = read('c17-elongation-net')
    ...
    assert added == {r['id']: r for r in read('alcohol-acetates')['reactions']}
    assert len(added) == 6 and not report['existing_equation_joins']
    for rid in added:
    ...
    assert len(report['targets']) == 6220 and report['summary']['balanced_equations'] == 18221
    assert report['summary']['net_status_counts'] == dict(Counter(t['net_status'] for t in report['targets']))
    assert {t['cannabisdb_id'] for t in report['targets'] if t['new_certificate']} == {'CDB000110','CDB000695'}
    assert {t['cannabisdb_id'] for t in report['targets'] if t['parent_net_status'] == 'no-net-producing-equation'
            and t['net_status'] == 'solver-reported-infeasible'} == {'CDB000192','CDB000283','CDB000321','CDB000585'}
