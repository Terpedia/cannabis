...
def test_full_alkane_model_obstructions_replay_every_allowed_direction():
    root=Path('data/reports'); read=lambda n:json.loads((root/n).read_bytes())
    report=read('phase1-alkane-obstructions.json'); current=read('phase1-alkane-net.json')
    ...
    assert report['summary']['balanced_equations']==len(reactions)==18203
