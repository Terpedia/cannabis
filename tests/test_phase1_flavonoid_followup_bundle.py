...
from cannabis_carbon.phase1_flavonoid_followup_bundle import SOURCES
from cannabis_carbon.phase1_reference_gap_bundle import build


def test_flavonoid_followup_export_is_lossless_and_does_not_promote(monkeypatch):
    monkeypatch.chdir(Path(__file__).resolve().parents[1])
    path = Path('data/reports/phase1-flavonoid-followup-bundle.json')
    ...
    assert Path('docs/data/flavonoid-followup-bundle.json').read_bytes() == path.read_bytes()
    rows = [json.loads(line) for line in Path('data/derived/phase1-flavonoid-followup-bundle.ndjson').read_text().splitlines()]
    assert len(rows) == 10
