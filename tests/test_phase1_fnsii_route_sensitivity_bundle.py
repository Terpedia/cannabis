...
from cannabis_carbon.phase1_fnsii_route_sensitivity_bundle import SOURCES
from cannabis_carbon.phase1_reference_gap_bundle import build


def test_fnsii_route_sensitivity_bundle_is_lossless(monkeypatch):
    monkeypatch.chdir(Path(__file__).resolve().parents[1])
    path = Path('data/reports/phase1-fnsii-route-sensitivity-bundle.json')
    ...
    assert Path('docs/data/fnsii-route-sensitivity-bundle.json').read_bytes() == path.read_bytes()
    rows = [json.loads(line) for line in Path('data/derived/phase1-fnsii-route-sensitivity-bundle.ndjson').read_text().splitlines()]
    assert len(rows) == len({(r['record_kind'], r['record_id']) for r in rows}) == 3
