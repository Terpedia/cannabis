...
from cannabis_carbon.phase1_fnsii_evidence_bundle import SOURCES
from cannabis_carbon.phase1_reference_gap_bundle import build


def test_fnsii_bundle_is_lossless_without_promotion(monkeypatch):
    monkeypatch.chdir(Path(__file__).resolve().parents[1])
    path = Path('data/reports/phase1-fnsii-evidence-bundle.json')
    ...
    assert Path('docs/data/fnsii-evidence-bundle.json').read_bytes() == path.read_bytes()
    rows = [json.loads(line) for line in Path('data/derived/phase1-fnsii-evidence-bundle.ndjson').read_text().splitlines()]
    assert len(rows) == len({(r['record_kind'], r['record_id']) for r in rows}) == 7
